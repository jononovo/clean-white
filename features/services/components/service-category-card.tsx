  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 mb-3 relative z-10">
        ...
        <Link href={category.categorySlug ? `/categories/${category.categorySlug}` : "#"}>
          <h4 className="font-display font-bold text-sm text-foreground hover:text-primary transition-colors">
            {category.label}
          </h4>
        </Link>
      </div>
      ...
      {providers.length > 0 && (
        <div className="flex flex-col gap-1 relative z-10">
          {providers.map((provider) => {
            const internalHref = provider.handle && provider.slug
              ? `/${provider.handle}/${provider.slug}`
              : null;
